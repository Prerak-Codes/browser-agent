...
function groupByLine(words: OCRDetection[]): OCRDetection[][] {
  const sorted = [...words].sort((a, b) => a.y - b.y);
  const lines: OCRDetection[][] = [];
  let currentLine: OCRDetection[] = sorted.length > 0 ? [sorted[0]] : [];

  for (let i = 1; i < sorted.length; i++) {
    const prev = currentLine[currentLine.length - 1];
    const verticalGap = Math.abs(sorted[i].y - prev.y);

    if (verticalGap > prev.height * 0.5) {
...
function detectEmails(ocrDetections: OCRDetection[]): SensitiveRegion[] {
  ...
  for (const detection of ocrDetections) {
    ...
    for (const match of matches) {
      const startIdx = detection.text.indexOf(match);
      const charWidth = detection.width / detection.text.length;

      results.push({
        type: "email",
        confidence: 0.95,
        x: detection.x + startIdx * charWidth,
        y: detection.y,
...
function detectPasswords(ocrDetections: OCRDetection[]): SensitiveRegion[] {
  const results: SensitiveRegion[] = [];
  const lines = groupByLine(ocrDetections);

  for (const line of lines) {
    for (let i = 0; i < line.length; i++) {
      const word = line[i].text.toLowerCase().replace(/[:\s]/g, "");
      const isKeyword = PASSWORD_KEYWORDS.some(
        (kw) => word === kw || word === kw.replace(/\s/g, "")
      );
      ...
      const valueText = remaining.map((w) => w.text).join(" ");
      ...
      results.push({
        ...
        confidence: 0.8,
        x: firstWord.x,
        y: Math.min(line[i].y, firstWord.y),
        width: lastWord.x + lastWord.width - firstWord.x,
        height: Math.max(line[i].height, firstWord.height),
        source: "rule",
...
function detectCreditCards(ocrDetections: OCRDetection[]): SensitiveRegion[] {
  ...
  for (const detection of ocrDetections) {
    ...
    for (const match of matches) {
      ...
      if (PHONE_REGEX.test(match)) continue;
